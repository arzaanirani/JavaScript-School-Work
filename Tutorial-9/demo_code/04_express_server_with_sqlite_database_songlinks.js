...
function findSongsPage(request, response, next){
        ...
        if(urlObj.query['composer']) {
		    ...
		    sql = "SELECT id, title FROM songs WHERE composer LIKE '%" + 
			          urlObj.query['composer'] + "%'"; 			
		}		
        if(urlObj.query['key']) {
		    console.log("finding key: " + urlObj.query['key']);
		    sql = "SELECT id, title FROM songs WHERE key LIKE '%" + 
			          urlObj.query['key'] + "%'"; 			
		}
        if(urlObj.query['title'] && urlObj.query['composer']) {
		    ...
		    sql = "SELECT id, title FROM songs WHERE title LIKE '%" + 
			          urlObj.query['title'] + "%' AND composer LIKE '%" + urlObj.query['composer'] + "%'"; 			
		}
		
        if(urlObj.query['composer'] && urlObj.query['key']) {
		    console.log("finding title: " + urlObj.query['composer']);
		    sql = "SELECT id, title FROM songs WHERE composer LIKE '%" + 
			          urlObj.query['composer'] + "%' AND key LIKE '%" + urlObj.query['key'] + "%'"; 			
		}
...
function addUser(request, response, next){
  var page = '<html><head><title>Login</title></head>' +
    '<body>' +
    '<form method="get">' +
    ...
    '</form>';
    response.write(page);
	db.run("INSERT INTO USERS VALUES ('"+urlObj.query.userid+"','"+urlObj.query.password+"' )");
	next();
}
